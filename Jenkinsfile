pipeline{
	...
	stages {
		...
		stage('Checkout'){
			steps{
				echo "------------>Checkout<------------"
				checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], gitTool: 'Git_Centos', submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'GitHub_javierosorio11', url: 'https://github.com/javierosorio11/pruebaTecnica.git']]])
			}
		}
	...
	post {
		failure {
			...
			mail( to: 'alvaro.osorio@ceiba.com.co' ,
